...
import './GuestTrasportationCounter.css';

const GuestTrasportationCounter = () => {
  const { guests, socketConnected } = useContext(DataContext);
  const userCount =()=>{
    return guests.reduce((acc, guest) => acc + (guest.guestInvited === 'YES' && guest.guestInvitationResponse && guest.guestInvitationDelivered &&  guest.guestTransportCount > 0) ? guest.guestTransportCount : 0);
  }
...
export default GuestTrasportationCounter; 